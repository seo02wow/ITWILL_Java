...
public class ManagerMenu {
	...
	public void selectManagerMenu5() {
			...
			while (true) {
				try {
					...
					if (menuSelect < 0 || menuSelect > 4) {
						System.out.println("▶▷▶▷ 범위를 벗어난 값입니다. MENUID(1 ~ 4) 입력 가능");
					} else {
						break;
					}
					
				} catch (NumberFormatException e) {
					System.out.println("▶▷▶▷ 문자입력불가. MENUID(1 ~ 4) 입력 가능");
				} 
			} 
	...
	public void MenuPrice() {
		...
		while (true) {
			try {
				...
				System.out.println("▶▷▶▷ 문자입력불가. 다시 입력하세요.");
			} 
		} 
	}
	...
	public void MenuName() {
		do {
			...
			if (menuName.isEmpty()) {
				...
				System.out.println("▶▷▶▷ 메뉴 이름은 한글과 영어 문자만 입력하세요.");
			}
		...
		while (result) { //true가 될 때까지 반복
			try {
				...
				if (menuId >= 1 && menuId <= 99) {
					...
					if (result) {
						System.out.println("▶▷▶▷ 중복 id값. 다시 입력하세요.");
					} else {
						break;
					}
				} else {
					System.out.println("▶▷▶▷ 범위를 벗어난 값입니다. MENUID(1 ~ 99) 입력 가능");
				}
			} catch (NumberFormatException e) {
				System.out.println("▶▷▶▷ 문자입력불가. MENUID(1 ~ 99) 입력 가능");
			} 
		}
	}
		...
		while (result) { //true가 될 때까지 반복
			try {
				...
				if (menuId >= 1 && menuId <= 99) {
					...
					if (result) {
						System.out.println("▶▷▶▷ 존재하지 않는 id값. 다시 입력하세요.");
					} else {
						break;
					}
				} else {
...
//------------------------ 4.주문상세조회 메소드 -------------------------
public void selectManagerMenu4() {
		...
		while (true) {
			try {
				...
				System.out.println("▶▷▶▷ 문자입력불가. MENUID(1 ~ 2) 입력 가능");
			} 
		} 
		...
		if (ordersdetailSelect == 1) {
			...
			for (JoinedDataVO vo : joinList) {
				System.out.println(vo.getOrderid() + "\t" + 
						...
						+ "\t" + vo.getCount() + "\t" + 
						(vo.getCount() * vo.getPrice())  + "\t" + vo.getOrderdate());
			}
		}
			...
			while (true) {
				try {
					...
					if (salesSelect < 0 || salesSelect > 3) {
						System.out.println("▶▷▶▷ 범위를 벗어난 값입니다. MENUID(1 ~ 3) 입력 가능");
					} else {
						break;
					}
					
				} catch (NumberFormatException e) {
					System.out.println("▶▷▶▷ 문자입력불가. MENUID(1 ~ 3) 입력 가능");
				} 
			} 
...
public int MenuIdContains() {
    ...
    while (true) {
       try {
          ...
          boolean result = dao.checkMenuIdExistence3(selectMenuId);
          if (!result) {
             System.out.println("▶▷▶▷ 존재하지 않는 id값. 다시 입력하세요.");
          } else {
             break;
