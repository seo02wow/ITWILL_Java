...
public class MenuDAO {
	...
	//SELECT : db에 특정 menuid가 존재하는지 확인
    // (주문하기에서 음료id 입력할 때 db에 존재하는 id값인지 확인)
    public boolean checkMenuIdExistence3(int selectMenuId) {
       Connection conn = null;
       PreparedStatement pstmt = null;
       ResultSet rs = null;
       ...
       try {
          ...
          String sql = "SELECT COUNT(*) FROM MENU WHERE MENUID = ?";
          pstmt = conn.prepareStatement(sql);
          pstmt.setInt(1, selectMenuId);
          rs = pstmt.executeQuery();

          if (rs.next()) {
             int count = rs.getInt(1);
             return count == 1; // =입력한 id값으로 데이터가 존재한다 
          }
